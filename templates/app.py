...
# === 🔥 НОВЫЙ МАРШРУТ: Создание пользователей владельцем ===
...
def create_user():
    ...
    if request.method == 'POST':
        ...
        # Генерация 7-значного числа для email (после @)
        email_suffix = str(random.randint(1000000, 9999999))  # 7 цифр

        # Генерация НЕЗАВИСИМОГО 8-значного пароля
        password = str(random.randint(10000000, 99999999))  # 8 цифр
        ...
        # Проверка уникальности email внутри компании
